...
def _response_usage_summary(response, elapsed=None):
    ...
    if elapsed and elapsed > 0:
        ...
        if throughput_tokens is not None:
            parts.append(f"rate={float(throughput_tokens) / elapsed:.2f}tok/s")
    if not parts:
        return "usage=unavailable"
    return "usage(" + ", ".join(parts) + ")"


def _start_progress_heartbeat(label, logging_fn, interval=LLM_PROGRESS_LOG_INTERVAL):
...
def get_json_response_from_llm(
    ...
    log = logging or print
    new_msg_history = [{"role": "user", "content": msg}]
    request_label = f"LLM JSON request model={model}"
    log(
        f"{request_label} started with prompt_chars={len(msg)} "
        f"history_messages={len(new_msg_history)}"
    )
    ...
    try:
        ...
        log(f"{request_label} failed after {time() - started:.2f}s: {exc}")
        raise
    finally:
        stop_event.set()
    ...
    log(
        f"{request_label} completed in {time() - started:.2f}s; "
        f"content_chars={len(_response_text_content(response))}; "
...
def get_response_from_llm(
    ...
    if msg_history is None:
        msg_history = []
    request_label = f"LLM request model={model}"
    log(
        f"{request_label} started with prompt_chars={len(msg)} "
        f"history_messages={len(msg_history)}"
    )
    stop_event, heartbeat_thread, started = _start_progress_heartbeat(
        request_label, log
    )
